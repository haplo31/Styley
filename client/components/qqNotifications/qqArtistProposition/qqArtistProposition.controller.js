...
angular.module('styleyApp')
  .controller('qqArtistProposition', function ($scope,data,$timeout,$http,$modalInstance) {
    $scope.message = 'Hello';
    $scope.data = data
	...
	$scope.acceptRequest = function(){
		$http.post('/api/qqsystem/responseartist',{data:data,response:'accept'}).success(function(){
            $modalInstance.close();
        });
	}
	$scope.declineRequest = function(){
		$http.post('/api/qqsystem/responseartist',{data:data,response:'refuse'}).success(function(){
            $modalInstance.close();
        });
	}	
	$timeout(function(){
